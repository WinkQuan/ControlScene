...
def cli_main():
    ...
    # 获取配置参数
    text = config_loader.get('text', '/root/autodl-tmp/data/Matterport3D/mp3d_skybox/e9zR4mvMWw7/blip3_stitched/test.txt')
    neg_text = config_loader.get('neg_text', '')
    campath_gen = config_loader.get('campath_gen', 'lowerscan')
    ...
    result_dir = f'/root/autodl-tmp/SceneDreamer360/logs/4142dlo4/predict/e9zR4mvMWw7_{text_dir_name}'
    

    # Enhance Pano_Image
    prompt_path = result_dir + '/prompt.txt'
    print('HERE!!!!,prompt_path is ',prompt_path)
    ...
    image_size = image_size # 按照输入参数调整输出图片的大小
    ...
    end_time = time.time()
    print('整体训练时间为：',end_time-start_time)
    '''
